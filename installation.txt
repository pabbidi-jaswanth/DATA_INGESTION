...
Fixes:
- Strict CSV fallbacks (only files whose names match: eligibility/links/documents).
- Require non-empty required fields; dedupe; validate URLs.
- 'UG links' shows only UG/ALL rows; similar for PG.
- Degree tokens map to LEVEL only; specializations map to PROGRAM.
- MSc/MCA relaxed-level fallback (PG -> ALL).
"""

import sqlite3, pathlib, re, csv, glob
...
def _is_http(url: str) -> bool:
    return bool(re.match(r"^https?://", (url or "").strip(), flags=re.I))
...
def _dedupe(items: List[Dict[str, Any]], keyfn) -> List[Dict[str, Any]]:
    seen = set(); out = []
    for it in items:
        k = keyfn(it)
        if k in seen: 
...
def _csv_scan(
    patterns: List[str],
    must_include_in_filename: List[str],
    required_columns: List[str],
) -> List[Dict[str, str]]:
    """Scan CSVs under Data/** that match both filename keywords and required columns."""
    rows: List[Dict[str, str]] = []
    for pat in patterns:
        for path in glob.glob(pat, recursive=True):
            name = pathlib.Path(path).name.lower()
            if must_include_in_filename and not any(k in name for k in must_include_in_filename):
                continue
            try:
                with open(path, "r", encoding="utf-8-sig", newline="") as f:
                    ...
                    if not set(required_columns).issubset(set([c.strip() for c in reader.fieldnames])):
                        continue
                    for r in reader:
                        row = {k: (r.get(k, "") or "").strip() for k in required_columns}
                        row["source_file"] = pathlib.Path(path).name
                        rows.append(row)
            except Exception:
                continue
    return rows
...
def _relaxed_levels_sequence(f: Dict[str, Any]) -> List[Optional[str]]:
    ...
    lv = f.get("level_like")
    if lv: seq.append(lv)
    if f.get("raw_degree") in {"msc","mca"} and "pg" not in seq:
        seq.append("pg")
    seq.append(None)  # final global
    return seq
...
def detect_structured_intent(q: str) -> str:
    ...
    if "block" in ql and any(k in ql for k in ["name","names","list","all","codes","code"]):
        return "blocks"
    if any(k in ql for k in ["hostel","room","mess","mh","lh","ac","non-ac","canteen","residence"]):
        return "hostel"
    ...
    # narrative flow
    if any(w in ql for w in ["application process","admission process","how to apply","refund","cancellation","deadline","last date","when","why","what","who","how"]):
        return "text"
    return "text"
...
def parse_filters(q: str) -> Dict[str, Any]:
    ...
    if any(k in ql for k in ["nri","foreign","international"]): f["category"] = "International"
    elif "indian" in ql: f["category"] = "Indian"

    deg = _find_degree_token(q); f["raw_degree"] = deg
    if deg == "btech": f["level_like"], f["prog_is_degree"] = "UG", True
    elif deg == "mtech": f["level_like"], f["prog_is_degree"] = "PG", True
    elif deg == "mca": f["level_like"], f["prog_is_degree"] = "mca", True
    elif deg == "msc": f["level_like"], f["prog_is_degree"] = "msc", True
...
def sql_eligibility(f: Dict[str,Any], _q: str) -> Dict[str,Any]:
    ...
    # CSV fallback (strict)
    if not rows_all:
        candidates = _csv_scan(
            ["Data/**/*.csv"],
            must_include_in_filename=["eligibility","criteria"],
            required_columns=["level","program","criteria"],
        )
        filtered = []
    ...
    return {"table":{"title":"Eligibility","columns":cols,"rows":[
        [r.get("level",""), r.get("program",""), r.get("criteria",""), r.get("source_file","")]
        for r in rows_all
    ]}}
...
def sql_documents(f: Dict[str,Any], _q: str) -> Dict[str,Any]:
    ...
    if not rows_all:
        candidates = _csv_scan(
            ["Data/**/*.csv"],
            must_include_in_filename=["document","documents","doc","submit"],
            required_columns=["level","program","item","details"],
        )
        filtered = []
        lv_seq = _relaxed_levels_sequence(f)
        def _match_level(s: str) -> bool:
            ...
            return False
        for r in candidates:
            if not r.get("item") and not r.get("details"):
                continue
            if not _match_level(r.get("level","")):
                continue
            if f["program_like"] and not f["prog_is_degree"]:
...
def sql_links(f: Dict[str,Any], _q: str) -> Dict[str,Any]:
    """
    links(level, program, name, url, source_file)
    Strict logic: if SQL empty/missing, only accept CSVs named like *links*.csv/*link*.csv/*url*.csv.
    Require valid http(s) URL; restrict by level (e.g., 'UG links' -> UG/ALL rows).
    """
    con = _mk()
    have_table = True
    try:
    ...
    if not rows:
        candidates = _csv_scan(
            ["Data/**/*.csv"],
            must_include_in_filename=["links","link","url"],
            required_columns=["level","program","name","url"],
        )
        filtered = []
        lv_seq = _relaxed_levels_sequence(f)
        def _match_level(s: str) -> bool:
        ...
        for r in candidates:
            ...
            if f["program_like"] and not f["prog_is_degree"]:
                if _norm_like(f["program_like"]).strip("%") not in _norm_text(r.get("program","")):
                    continue
            filtered.append(r)
        # dedupe on (level, program, name, url)
        rows = _dedupe(filtered, keyfn=lambda x: (x.get("level",""), x.get("program",""), x.get("name",""), x.get("url","")))

    cols = ["Level","Program","Name","URL","Source"]
    return {"table":{"title":"Links","columns":cols,"rows":[
        [r.get("level",""), r.get("program",""), r.get("name",""), r.get("url",""), r.get("source_file","")]
        for r in rows
    ]}}
